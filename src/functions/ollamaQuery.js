...
export async function QueryChat(Year, competitions, target, userPrmpt = ``) {
	...
	const Assistpmpt = {
      ...
      <判斷方式>
				...
				<甄選一般生錄取率 FORMULA="(甄選一般生招生名額 - 甄選一般生名額空缺) / 甄選一般生招生名額">
					錄取率較高代表著代表名額幾乎都被填滿（報到狀況良好）以及 學校或科系吸引力高，學生願意就讀如果錄取率低表示有許多空缺，招生不理想或學校或科系競爭力較低，可能招生困難。
				</甄選一般生錄取率>
				<R_score FORMULA="當年度的所有比賽(各校系為選手，以學生的去向為輸贏，採取一般生正取及一般生備取且錄取為有效資料)使用 Trueskill來計算">
